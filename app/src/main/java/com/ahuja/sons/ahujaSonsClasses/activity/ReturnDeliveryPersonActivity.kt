...
import com.ahuja.sons.databinding.ActivityReturnDeliveryPersonBinding
...
class ReturnDeliveryPersonActivity : AppCompatActivity() {
    lateinit var binding: ActivityReturnDeliveryPersonBinding
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding = ActivityReturnDeliveryPersonBinding.inflate(layoutInflater)
...
//                binding.btnSubmit.visibility = View.VISIBLE
        ...
        if (modelData.OrderRequest!!.SapOrderId.isNotEmpty()) {
            binding.apply {
                ...
                itemDetailView.setOnClickListener {
                    var intent = Intent(this@ReturnDeliveryPersonActivity, ItemDetailActivity::class.java)
    ...
    private fun callSurgeryPersonDetailApi() {
        ...
        call.enqueue(object : Callback<SurgeryPersonNameListModel?> {
                ...
                if (response.body()!!.status == 200) {
                    ...
                    if (data.size > 0){
                        ...
                        binding.apply {
                            ...
                            rvSurgeryPersonListName.layoutManager = LinearLayoutManager(this@ReturnDeliveryPersonActivity, LinearLayoutManager.VERTICAL, false)
                    ...
                    Global.warningmessagetoast(this@ReturnDeliveryPersonActivity, response.body()!!.message);
            ...
            override fun onFailure(call: Call<SurgeryPersonNameListModel?>, t: Throwable) {
                binding.loadingBackFrame.visibility = View.GONE
                binding.loadingView.stop()
                Toast.makeText(this@ReturnDeliveryPersonActivity, t.message, Toast.LENGTH_SHORT).show()
            }
        })
                        ...
                        if (data.StartAt.isNotEmpty()){
                            ...
                            if (!proofData.isNullOrEmpty()){
                                ...
                                if (mArrayUriList.size > 0) {
                                    ...
                                    val linearLayoutManager = LinearLayoutManager(this@ReturnDeliveryPersonActivity, LinearLayoutManager.HORIZONTAL, false)
                                    val adapter = PreviousImageViewAdapter(this@ReturnDeliveryPersonActivity, mArrayUriList, arrayOf(), arrayListOf())
    ...
    private fun setupDeliveryDetailRecyclerview() = binding.rvDeliveryDetail.apply {
        ...
        layoutManager = LinearLayoutManager(this@ReturnDeliveryPersonActivity)
        ...
        call.enqueue(object : Callback<AllWorkQueueResponseModel?> {
            override fun onResponse(call: Call<AllWorkQueueResponseModel?>, response: Response<AllWorkQueueResponseModel?>) {
                if (response.body()!!.status == 200) {
                    ...
                    Log.e("data", response.body()!!.data.toString())
                    Global.successmessagetoast(this@ReturnDeliveryPersonActivity, "Picture Uploaded Success")
                    bottomSheetDialog.dismiss()
...
//                    binding.btnSubmit.visibility = View.VISIBLE
    ...
    private fun getMyCurrentLocation(type: String) {
        ...
        val locationManager = this@ReturnDeliveryPersonActivity.getSystemService(Context.LOCATION_SERVICE) as LocationManager
                ...
                if (location != null) {
                    ...
                    val locationCallback = object : LocationCallback() {
                        override fun onLocationResult(locationResult: LocationResult) {
                            ...
                            val geocoder = Geocoder(this@ReturnDeliveryPersonActivity, Locale.getDefault())
                            var addresses: List<Address>? = null
...
//                    startTimer()
                    ...
                    binding.loadingBackFrame.visibility = View.GONE
                    binding.loadingView.stop()
                    Global.warningmessagetoast(this@ReturnDeliveryPersonActivity, response.body()!!.errors);
    ...
    private val timerRunnable = object : Runnable {
        override fun run() {
            if (isRunning) {
                ...
                binding.tvReturnCountText.text = String.format("%02d:%02d:%02d", hours, minutes, seconds)
    ...
    private fun runTimer() {
        ...
        handler.post(object : Runnable {
            override fun run() {
                ...
                binding.tvReturnCountText.text = time
