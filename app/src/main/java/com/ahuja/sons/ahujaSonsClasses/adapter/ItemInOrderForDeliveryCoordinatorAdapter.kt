...
class ItemInOrderForDeliveryCoordinatorAdapter :
    ListAdapter<LocalWorkQueueData, ItemInOrderForDeliveryCoordinatorAdapter.OrderViewHolder>(OrderDiffCallback()) {


    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): OrderViewHolder {
        val binding =
            ItemOfDeliveryCoordinatorOrderItemBinding.inflate(LayoutInflater.from(parent.context), parent, false)
    ...
    class OrderViewHolder(private val binding: ItemOfDeliveryCoordinatorOrderItemBinding) :
        ...
        fun bind(order: LocalWorkQueueData) {
            binding.tvItemName.text = order.orderName

            binding.tvItemQuantity.text = "12,11,13"
            binding.tvUOM.text = "${order.omsID}"
            binding.tvQty.text = "${order.status}"
        }
    }
