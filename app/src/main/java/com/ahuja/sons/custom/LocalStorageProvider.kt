...
class LocalStorageProvider : DocumentsProvider() {
    ...
    override fun queryRoots(projection: Array<String>): Cursor {
        ...
        // Add Home directory
        val homeDir: File = Environment.getExternalStorageDirectory()
        val row = result.newRow()
        // These columns are required
        row.add(Root.COLUMN_ROOT_ID, homeDir.getAbsolutePath())
        row.add(Root.COLUMN_DOCUMENT_ID, homeDir.getAbsolutePath())
        row.add(Root.COLUMN_TITLE, context!!.getString(R.string.save))
        row.add(Root.COLUMN_FLAGS, Root.FLAG_LOCAL_ONLY or Root.FLAG_SUPPORTS_CREATE)
        row.add(Root.COLUMN_ICON, R.drawable.ic_mike)
    ...
    override fun openDocumentThumbnail(documentId: String?, sizeHint: Point,
        ...
        val targetHeight: Int = 2 * sizeHint.y
        val targetWidth: Int = 2 * sizeHint.x
    ...
    override fun getDocumentType(documentId: String): String {
        ...
        val lastDot: Int = file.getName().lastIndexOf('.')
        if (lastDot >= 0) {
            val extension: String = file.getName().substring(lastDot + 1)
